...
- [x] **TASK-014: 렌더링 및 번들 최적화** ✅ - 완료
  - ✅ `React.memo`로 TimingCards, BudgetBar 불필요한 리렌더링 방지 (`components/parking/timing-cards.tsx`, `budget-bar.tsx`)
  - ✅ `useMemo`로 `calculateFeeResult`, `calculateTimingOptions`, `estimateTimeToBudget` 계산 결과 메모이제이션 (`components/parking/parking-calculator.tsx`)
  - ✅ `next/font`로 Geist 웹폰트 최적화 (`display: 'swap'` 적용, `app/layout.tsx`)
  - ✅ `next/image` 불필요 (이미지 에셋 없음, lucide-react 인라인 SVG 사용)
  - ✅ `next/dynamic` 불필요 (컴포넌트 규모 대비 효과 미미, 현재 번들 구조 적정)

- [x] **TASK-015: Web Vitals 측정 및 개선** ✅ - 완료
  - ✅ `useReportWebVitals` 훅으로 Web Vitals 수집 (`components/web-vitals.tsx`)
  - ✅ `@vercel/analytics` 연동, `<Analytics />` 컴포넌트로 페이지뷰 + Web Vitals 자동 수집 (`app/layout.tsx`)
  - ✅ `.lighthouserc.js` 생성: 성능/접근성/모범사례/SEO 각 90점 기준점 설정
  - ✅ Supabase 쿼리 최적화: 페이지당 필요한 쿼리만 조회, N+1 발생 구조 없음
  - ✅ 서버 컴포넌트 데이터 패칭 병렬화: `Promise.all` 적용 (`app/protected/parking/page.tsx`)
...
- [x] **TASK-017: Vercel 프로덕션 배포 설정** ✅ - 완료
  - ✅ `vercel.json` 생성: 보안 헤더 6종 설정 (CSP, X-Frame-Options, X-Content-Type-Options, HSTS, Referrer-Policy, Permissions-Policy)
  - ✅ CSP: Next.js 인라인 스크립트, Supabase 도메인, Vercel Analytics 도메인 허용
  - ✅ `README.md` 전면 교체: 프로젝트 개요, 기술 스택, 로컬 개발 설정, 환경 변수 설명표 포함
  - ✅ Vercel 배포 단계별 가이드 문서화 (GitHub 연동, 환경 변수, Preview 배포)
  - ✅ Supabase Auth Redirect URL 설정 방법 문서화 (Site URL, 와일드카드 Preview URL)
  - ✅ 아키텍처 다이어그램 (ASCII) 및 배포 확인 체크리스트 포함

- [x] **TASK-018: 에러 모니터링 및 운영 설정** ✅ - 완료
  - ✅ `@sentry/nextjs` v10, `@vercel/analytics` 패키지 설치
  - ✅ `instrumentation.ts` 생성: Next.js 15 계측 진입점, `onRequestError` 핸들러 연동
  - ✅ `sentry.client.config.ts` 생성: 클라이언트 에러 수집 + 세션 리플레이 설정
  - ✅ `sentry.server.config.ts` 생성: 서버 사이드 에러 수집 설정
  - ✅ `sentry.edge.config.ts` 생성: 엣지 런타임 에러 수집 설정
  - ✅ `next.config.ts`: `withSentryConfig` 래핑, 소스맵 자동 삭제, 로거 트리쉐이킹 설정
  - ✅ `components/web-vitals.tsx`: `useReportWebVitals` 훅 방식으로 교체 (개발 환경 콘솔 출력)
  - ✅ `app/layout.tsx`: `<Analytics />` 컴포넌트 추가 (페이지뷰 + Web Vitals 자동 수집)
  - ✅ `README.md`에 모니터링 설정 가이드 (Sentry, Vercel Analytics, Lighthouse CI) 포함
...
_최종 업데이트: 2026-03-04_ | **📊 진행 상황**: Phase 0–8 전체 완료 (19/19 Tasks 완료) 🎉
