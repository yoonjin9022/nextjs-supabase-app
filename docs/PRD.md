...
### F007 - 공개 랜딩 페이지 및 인증 홈 대시보드

**공개 랜딩 페이지 (`/`)**:

- 미인증 사용자에게 앱을 소개하는 ParkingMeter 브랜드 화면
- 핵심 기능 3가지 소개 카드 (실시간 요금 계산, 출차 타이밍 비교, 예산 관리)
- 시작하기(회원가입) / 로그인 CTA 버튼
- `max-w-[430px]` 모바일 퍼스트 레이아웃

**인증된 홈 대시보드 (`/protected`)**:

- 로그인 후 하단 탭 "홈" 진입 시 표시되는 대시보드
- 사용자 환영 메시지 (이메일 기반)
- 진행 중인 세션 카드: 있으면 계산기 이동 버튼, 없으면 주차 시작 버튼
- 이번 달 주차비 요약 (총 요금, 횟수, 평균)
...
```
app/
├── page.tsx                      # 공개 랜딩 페이지 (ParkingMeter 소개 + 로그인/회원가입 CTA)

app/protected/
├── page.tsx                      # 인증된 홈 대시보드 (환영 메시지 + 세션 현황 + 이번 달 요약)
